...
class Cursor(mariadb._mariadb.cursor):
            ...
            for i in range(0, len(self._keys)):
                if self._keys[i] not in self._data:
                    raise mariadb.ProgrammingError(
                        "Dictionary doesn't contain" " key '%s'" % self._keys[i]
                    )
        else:
            # check if number of place holders matches the number of
            # supplied elements in data tuple
            if self._paramlist and (
                (not self._data and len(self._paramlist) > 0)
                or (len(self._data) != len(self._paramlist))
            ):
                raise mariadb.ProgrammingError(
    ...
    def executemany(self, statement, parameters):
        ...
        #       rowcount updating
        if not (
            self.connection.extended_server_capabilities
    ...
    def scroll(self, value: int, mode="relative"):
        ...
        if mode != "absolute" and mode != "relative":
            raise mariadb.ProgrammingError(
                "Invalid or unknown scroll " "mode specified."
            )
        ...
        if mode == "relative":
            if self.rownumber + value < 0 or self.rownumber + value > self.rowcount:
