...
class ConnectionPool(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs and "pool_name" in kwargs:
            ...
            if kwargs["pool_name"] in mariadb._CONNECTION_POOLS:
                raise mariadb.ProgrammingError(
                    "Pool '%s' already exists" % kwargs["pool_name"]
                )
        else:
        ...
        self._pool_args["reset_connection"] = bool(
            kwargs.get("pool_reset_connection", True)
        )
        self._pool_args["validation_interval"] = int(
            kwargs.get("pool_validation_interval", 500)
        )

        # validate pool size (must be in range between 1 and MAX_POOL_SIZE)
        if not (0 < self._pool_args["size"] <= MAX_POOL_SIZE):
            raise mariadb.ProgrammingError(
                "Pool size must be in range of " "1 and %s" % MAX_POOL_SIZE
            )
    ...
    def __repr__(self):
        if self.__closed:
            return (
                "<mariadb.connectionPool.ConnectionPool object (closed) "
                "at %s>" % (hex(id(self)),)
            )
        else:
            return (
                "<mariadb.connectionPool.ConnectionPool object (name=%s) "
    ...
    def add_connection(self, connection=None):
        ...
        if not self._conn_args:
            raise mariadb.PoolError(
                "Couldn't get configuration for pool %s" % self._pool_args["name"]
            )
        ...
        if connection is not None and not isinstance(
            ...
            raise mariadb.ProgrammingError(
                "Passed parameter is not a " "connection object"
            )

        if connection is None and len(self._conn_args) == 0:
            raise mariadb.PoolError(
                "Can't get configuration for pool %s" % self._pool_args["name"]
            )

        total = len(self._connections_free + self._connections_used)
        if total >= self._pool_args["size"]:
            raise mariadb.PoolError(
                "Can't add connection to pool %s: "
                "No free slot available (%s)." % (self._pool_args["name"], total)
            )
