...
class Connection(mariadb._mariadb.connection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "host" in kwargs:
            ...
            if (
                version.Version(mariadb.mariadbapi_version) < version.Version("3.3.0")
                and "," in host
            ):
                raise mariadb.ProgrammingError(
                    "Host failover list requires "
    ...
    def cursor(self, cursorclass=mariadb.cursors.Cursor, **kwargs):
        ...
        if not isinstance(cursor, mariadb._mariadb.cursor):
            raise mariadb.ProgrammingError(
                "%s is not an instance of " "mariadb.cursor" % cursor
            )
        return cursor
    ...
    class xid(tuple):
        ...
        def __new__(self, format_id, transaction_id, branch_qualifier):
            if not isinstance(format_id, int):
                raise mariadb.ProgrammingError(
                    "argument 1 must be int, " "not %s", type(format_id).__name__
                )
            if not isinstance(transaction_id, str):
                raise mariadb.ProgrammingError(
                    "argument 2 must be str, " "not %s", type(transaction_id).__mane__
                )
            if not isinstance(branch_qualifier, str):
                raise mariadb.ProgrammingError(
                    "argument 3 must be str, " "not %s", type(transaction_id).__name__
                )
    ...
    def tpc_begin(self, xid):
        ...
        if type(xid).__name__ != "xid":
            raise mariadb.ProgrammingError(
                "argument 1 must be xid " "not %s", type(xid).__name__
            )
